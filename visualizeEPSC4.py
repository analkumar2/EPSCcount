...
def alpha2(t, A,B,C,tau1,tau2):
	ttr = np.array([max(0,ttt-B) for ttt in t])
	Re = C-A*ttr/(tau1-tau2)*(np.exp(-ttr/tau1) - np.exp(-ttr/tau2))
	# Re[Re>C] = C
	return Re
...
k=0
for p in potentialEPSCpeaks:
	# paramfitted,error = bcf.brute_scifit(alpha2, t, V[peaks[2]-100:peaks[2]+300],  [[1,0,min(V),0.0005], [100,400/fs,max(V),0.005]], ntol = 1000, returnnfactor = 0.01, maxfev = 1000, printerrors=False) #actual fitting to the 20th peak
	paramfitted,error = bcf.brute_scifit(alpha2, t, V[p-100:p+300],  [[0.001,0,min(V),0.0005,0.0005], [0.050,400/fs,max(V),0.005,0.005]], ntol = 1000, returnnfactor = 0.01, maxfev = 1000, printerrors=False) #actual fitting to the 20th peak
	print(p,paramfitted,error)
	A,B,C,tau1,tau2 = paramfitted
	# plt.plot(T[p-100:p+300], alpha2(t,*paramfitted))
	if (abs(p-actualEPSCidxlist)<20).any():
		print(T[p],'True')
		A1_list.append(A)
		B1_list.append(B)
		tau11_list.append(tau1)
		tau21_list.append(tau2)
		error1_list.append(error)
		V1_list.append(V[p-100:p+300])
		k=k+1
		# plt.plot(T[p], V[p], "o")
		plt.plot(t, alpha2(t,A,0,0,tau1,tau2), color='blue')
	else:
		print(T[p], 'False')
		A0_list.append(A)
		B0_list.append(B)
		tau10_list.append(tau1)
		tau20_list.append(tau2)
		error0_list.append(error)
		V0_list.append(V[p-100:p+300])
		# plt.plot(T[p], V[p], "x")
		plt.plot(t, alpha2(t,A,0,0,tau1,tau2), color='green')



# plt.plot(T[peaks[2]-100:peaks[2]+300], alpha2(t,40,0.004,0.085,0.002))
# plt.xlim(2,2.05)
print(k)
plt.show()

# fig1,axs1 = plt.subplots(1,1)
# axs1.plot(A1_list, tau11_list, 'o')
# axs1.plot(A0_list, tau10_list, 'x')

# fig1,axs1 = plt.subplots(1,1)
# axs1.plot(A1_list, tau21_list, 'o')
# axs1.plot(A0_list, tau20_list, 'x')
...
# axs3.plot(B1_list, tau11_list, 'o')
# axs3.plot(B0_list, tau10_list, 'x')

# fig3,axs3 = plt.subplots(1,1)
# axs3.plot(B1_list, tau21_list, 'o')
# axs3.plot(B0_list, tau20_list, 'x')
# # plt.show()
...
# ax.scatter(A1_list, tau11_list, B1_list, c='blue')
# ax.scatter(A0_list, tau10_list, B0_list, c='green')
# plt.show()
...
############ PCA on alpha2 fitted parameters ###########
pca = PCA(n_components=2)
parameters1_list = np.transpose([A1_list,tau11_list,tau21_list])
parameters0_list = np.transpose([A0_list,tau10_list,tau20_list])
...
for p in actualEPSCidxlist:
	paramfitted,error = bcf.brute_scifit(alpha2, t, V[p-100:p+300],  [[0.001,0,min(V),0.0005,0.0005], [0.050,400/fs,max(V),0.005,0.005]], ntol = 1000, returnnfactor = 0.01, maxfev = 1000, printerrors=False) 
	print(paramfitted,error)
	A,B,C,tau1,tau2 = paramfitted
	plt.plot(t, V[p-100:p+300], color='black')
	plt.plot(t, alpha2(t,A,B,C,tau1,tau2), color='blue')
	plt.show()